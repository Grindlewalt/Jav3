...
async def test_csrf_check_still_refuses_foreign_origin(fake_lan, tmp_env):
    ...
    require_same_origin(req("http://jarvis.local:8000"))    # own mDNS name
...
async def test_mdns_registers_and_unregisters(fake_lan, monkeypatch):
    ...
    info = calls[0][1]
    assert info["a"] == ("_http._tcp.local.", "jarvis._http._tcp.local.")
    assert info["server"] == "jarvis.local." and info["port"] == settings.lan_port
    await lan.stop()
