...
- **Short-lived.** TTL_SECONDS on the monotonic clock (the Pi has no RTC; an
  NTP step must not stretch a code's life or a lockout); expired tickets are
  swept on every mint/redeem.
- **Bounded.** At most MAX_LIVE outstanding tickets per user, MAX_TOTAL
  overall; minting past a user's cap drops THAT user's oldest, so one account
  (or a looping tab) cannot evict another's code in flight.
- **Cancellable.** `cancel(by)` drops a user's live codes — Settings calls it
  when the operator dismisses the code.

State is in memory. A restart forgets every outstanding code, which is right
for a ten-minute thing. It also means ONE process: a second worker would hold
a different ticket store and throttle (main.py refuses WEB_CONCURRENCY > 1).

Throttling is on MISSES only. A valid code always redeems: the handler looks
the code up first and consults the throttle only when the lookup failed. With
256-bit codes the throttle is DoS hygiene (it tells a noisy client to stop),
never guessing defence, so there is nothing to gain from letting an
attacker's misses lock the operator's valid code out — which is exactly what
checking the budget first allowed (shared NAT / ssh tunnel / 50 LAN aliases).

The throttle keys on the TCP peer (`request.client.host`), never on
X-Forwarded-For/CF-Connecting-IP: the server is LAN-first with nothing trusted
in front, so those headers are attacker-chosen. The systemd unit runs uvicorn
with --no-proxy-headers so uvicorn does not rewrite client.host from them
either.
"""
...
MAX_LIVE = 16                    # per user
MAX_TOTAL = 256                  # all users; memory bound only
...
_WRONG_PER_PEER = 10             # misses per peer per window before misses 429
_WRONG_GLOBAL = 500              # all peers
_CALLS_WINDOW = 60.0             # short-window miss rate (bursts)
_CALLS_PER_PEER = 30
_CALLS_GLOBAL = 600
...
def mint(name: str = "", by: str = "", now: float | None = None) -> tuple[str, Ticket]:
    ...
    _sweep(now)
    by = (by or "")[:64]
    mine = [t for t in _tickets.values() if t.by == by]
    while len(mine) >= MAX_LIVE:
        oldest = min(mine, key=lambda t: t.created)
        mine.remove(oldest)
        del _tickets[oldest.digest]
    while len(_tickets) >= MAX_TOTAL:
    ...
    t = Ticket(digest=_digest(code), name=(name or "").strip()[:64],
               by=by, created=now, expires=now + TTL_SECONDS)
    _tickets[t.digest] = t
    return code, t
...
def cancel(by: str, now: float | None = None) -> int:
    """Drop every live code minted by `by` (Settings "Done" / leaving the
    page). Returns how many were dropped."""
    _sweep(_now(now))
    gone = [d for d, t in _tickets.items() if t.by == (by or "")[:64]]
    for d in gone:
        del _tickets[d]
    return len(gone)
...
def _sweep_throttle(now: float) -> None:
    ...
    global _last_sweep
    if _last_sweep is not None and now - _last_sweep < 60:
        return
    _last_sweep = now
...
def throttle(peer: str, now: float | None = None) -> None:
    """On a MISS, before charging it. Raises TooMany when the peer (or
    everyone together) has spent the miss budget. Never called for a code
    that redeemed — see the module docstring."""
    now = _now(now)
    _sweep_throttle(now)
